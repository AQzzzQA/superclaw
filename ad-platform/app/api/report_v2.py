...
async def get_daily_report(
    ...
    try:
        # 构建查询
        query = db.query(DailyReport).filter(DailyReport.tenant_id == current_user.tenant_id)
        ...
        if start_date:
            query = query.filter(DailyReport.stat_date >= datetime.strptime(start_date, "%Y-%m-%d").date())
        if end_date:
            query = query.filter(DailyReport.stat_date <= datetime.strptime(end_date, "%Y-%m-%d").date())
...
async def get_trend_report(
    ...
    try:
        ...
        if start_date:
            reports = reports.filter(DailyReport.stat_date >= datetime.strptime(start_date, "%Y-%m-%d").date())
        if end_date:
            reports = reports.filter(DailyReport.stat_date <= datetime.strptime(end_date, "%Y-%m-%d").date())
        ...
        data = [
            {
                ...
                "CPM": r.cpm,
                "CPC": r.cpc,
                "CPA": f"{(r.cost / (r.convert or 1)):.2f}" if r.convert and r.cost else "0.00",
            }
            for r in reports
        ]
...
async def get_report_history(
    ...
    try:
        ...
        reports = db.query(DailyReport).filter(
            DailyReport.advertiser_id == advertiser_id
        ).order_by(DailyReport.stat_date.desc()).all()
