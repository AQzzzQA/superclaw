    ...
    try:
        ...
        for conv in request.conversions:
            conversions_data.append({
                ...
                "conversion_time": int(datetime.fromisoformat(conv.conversion_time).timestamp()),
                "value": conv.value,
            })
...
async def query_conversions(
    ...
    try:
        ...
        if start_date:
            query = query.filter(Conversion.conversion_time >= datetime.strptime(start_date, "%Y-%m-%d").timestamp())
        if end_date:
            query = query.filter(Conversion.conversion_time <= datetime.strptime(end_date, "%Y-%m-%d").timestamp())
        if conversion_type:
        ...
        data = [
            {
                ...
                "conversion_time": datetime.fromtimestamp(c.conversion_time).isoformat(),
                "value": c.value,
            }
            for c in results
        ]
