...
class AlertRuleService:
    ...
    def create(db: Session, obj_in: AlertRuleCreate, tenant_id: int) -> AlertRule:
        """创建预警规则"""
        try:
            db_obj = AlertRule(tenant_id=tenant_id, **obj_in.dict())
            db.add(db_obj)
            db.commit()
            db.refresh(db_obj)
            return db_obj
    ...
    def get(db: Session, id: int, tenant_id: int) -> Optional[AlertRule]:
        ...
        return (
            db.query(AlertRule)
            .filter(and_(AlertRule.id == id, AlertRule.tenant_id == tenant_id))
            .first()
        )
    ...
    @staticmethod
    def update(db: Session, db_obj: AlertRule, obj_in: AlertRuleUpdate) -> AlertRule:
        """更新预警规则"""
        try:
    ...
    def trigger_alert(db: Session, rule: AlertRule, current_value: float) -> AlertEvent:
        ...
        try:
            ...
            if rule.rule_type == "cost_alert":
                ...
                # 异常检测逻辑（简化版）
                is_triggered = (
                    abs(current_value - rule.threshold) > rule.threshold * 0.5
                )
...
class AlertEventService:
    ...
    def create(db: Session, obj_in: AlertEventCreate, tenant_id: int) -> AlertEvent:
        """创建预警事件"""
        try:
            db_obj = AlertEvent(tenant_id=tenant_id, **obj_in.dict())
            db.add(db_obj)
            db.commit()
            db.refresh(db_obj)
            return db_obj
    ...
    def get(db: Session, id: int, tenant_id: int) -> Optional[AlertEvent]:
        ...
        return (
            db.query(AlertEvent)
            .filter(and_(AlertEvent.id == id, AlertEvent.tenant_id == tenant_id))
            .first()
        )
    ...
    def get_multi(
        ...
        return (
            query.order_by(AlertEvent.created_at.desc()).offset(skip).limit(limit).all()
        )
...
class RealtimeMetricsService:
    ...
    def get_metrics(
        ...
        return (
            ...
            .order_by(RealtimeMetrics.metric_time.desc())
            .all()
        )
    ...
    def get_aggregated_metrics(
        ...
        total_show = (
            query.with_entities(func.sum(RealtimeMetrics.show_count)).scalar() or 0
        )
        total_click = (
            query.with_entities(func.sum(RealtimeMetrics.click_count)).scalar() or 0
        )
        total_convert = (
            query.with_entities(func.sum(RealtimeMetrics.convert_count)).scalar() or 0
        )
