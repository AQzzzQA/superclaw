...
class AudienceTargetingService:
    ...
    @staticmethod
    def create(
        db: Session, obj_in: AudienceTargetingCreate, tenant_id: int
    ) -> AudienceTargeting:
        """创建人群定向"""
        try:
            # 将 JSON 对象转换为字符串存储
            targeting_value_json = json.dumps(
                obj_in.targeting_value, ensure_ascii=False
            )
    ...
    def get(db: Session, id: int, tenant_id: int) -> Optional[AudienceTargeting]:
        """获取单个人群定向"""
        return (
            db.query(AudienceTargeting)
            .filter(
                and_(
                    AudienceTargeting.id == id, AudienceTargeting.tenant_id == tenant_id
                )
            )
            .first()
        )
    ...
    @staticmethod
    def update(
        db: Session, db_obj: AudienceTargeting, obj_in: AudienceTargetingUpdate
    ) -> AudienceTargeting:
        """更新人群定向"""
        try:
            ...
            if "targeting_value" in update_data:
                update_data["targeting_value"] = json.dumps(
                    update_data["targeting_value"], ensure_ascii=False
                )
...
class DeviceTargetingService:
    ...
    @staticmethod
    def create(
        db: Session, obj_in: DeviceTargetingCreate, tenant_id: int
    ) -> DeviceTargeting:
        """创建设备定向"""
        try:
    ...
    def get(db: Session, id: int, tenant_id: int) -> Optional[DeviceTargeting]:
        """获取单个设备定向"""
        return (
            db.query(DeviceTargeting)
            .filter(
                and_(DeviceTargeting.id == id, DeviceTargeting.tenant_id == tenant_id)
            )
            .first()
        )
    ...
    def get_multi(
        ...
        """获取设备定向列表"""
        query = db.query(DeviceTargeting).filter(DeviceTargeting.tenant_id == tenant_id)
    ...
    @staticmethod
    def update(
        db: Session, db_obj: DeviceTargeting, obj_in: DeviceTargetingUpdate
    ) -> DeviceTargeting:
        """更新设备定向"""
        try:
            update_data = obj_in.dict(exclude_unset=True)
...
class GeoTargetingService:
    ...
    def get(db: Session, id: int, tenant_id: int) -> Optional[GeoTargeting]:
        """获取单个地域定向"""
        return (
            db.query(GeoTargeting)
            .filter(and_(GeoTargeting.id == id, GeoTargeting.tenant_id == tenant_id))
            .first()
        )
    ...
    def get_multi(
        ...
        """获取地域定向列表"""
        query = db.query(GeoTargeting).filter(GeoTargeting.tenant_id == tenant_id)
    ...
    @staticmethod
    def update(
        db: Session, db_obj: GeoTargeting, obj_in: GeoTargetingUpdate
    ) -> GeoTargeting:
        """更新地域定向"""
        try:
            update_data = obj_in.dict(exclude_unset=True)
            ...
            if "geo_list" in update_data:
                update_data["geo_list"] = json.dumps(
                    update_data["geo_list"], ensure_ascii=False
                )
...
class TimeTargetingService:
    ...
    @staticmethod
    def create(
        db: Session, obj_in: TimeTargetingCreate, tenant_id: int
    ) -> TimeTargeting:
        """创建时间定向"""
        try:
            # 将时间配置转换为 JSON 字符串存储
    ...
    def get(db: Session, id: int, tenant_id: int) -> Optional[TimeTargeting]:
        """获取单个时间定向"""
        return (
            db.query(TimeTargeting)
            .filter(and_(TimeTargeting.id == id, TimeTargeting.tenant_id == tenant_id))
            .first()
        )
    ...
    def get_multi(
        ...
        """获取时间定向列表"""
        query = db.query(TimeTargeting).filter(TimeTargeting.tenant_id == tenant_id)
    ...
    @staticmethod
    def update(
        db: Session, db_obj: TimeTargeting, obj_in: TimeTargetingUpdate
    ) -> TimeTargeting:
        """更新时间定向"""
        try:
            update_data = obj_in.dict(exclude_unset=True)
            ...
            if "time_config" in update_data:
                update_data["time_config"] = json.dumps(
                    update_data["time_config"], ensure_ascii=False
                )
...
class EnvironmentTargetingService:
    ...
    @staticmethod
    def create(
        db: Session, obj_in: EnvironmentTargetingCreate, tenant_id: int
    ) -> EnvironmentTargeting:
        """创建环境定向"""
        try:
    ...
    @staticmethod
    def update(
        db: Session, db_obj: EnvironmentTargeting, obj_in: EnvironmentTargetingUpdate
    ) -> EnvironmentTargeting:
        """更新环境定向"""
        try:
            update_data = obj_in.dict(exclude_unset=True)
