...
class BiddingStrategyService:
    ...
    @staticmethod
    def create(
        db: Session, obj_in: BiddingStrategyCreate, tenant_id: int
    ) -> BiddingStrategy:
        """创建出价策略"""
        try:
    ...
    def get(db: Session, id: int, tenant_id: int) -> Optional[BiddingStrategy]:
        """获取单个出价策略"""
        return (
            db.query(BiddingStrategy)
            .filter(
                and_(BiddingStrategy.id == id, BiddingStrategy.tenant_id == tenant_id)
            )
            .first()
        )
    ...
    def get_multi(
        ...
        """获取出价策略列表"""
        query = db.query(BiddingStrategy).filter(BiddingStrategy.tenant_id == tenant_id)
    ...
    @staticmethod
    def update(
        db: Session, db_obj: BiddingStrategy, obj_in: BiddingStrategyUpdate
    ) -> BiddingStrategy:
        """更新出价策略"""
        try:
    ...
    def activate(db: Session, id: int, tenant_id: int) -> BiddingStrategy:
        ...
        try:
            ...
            ).update({"is_enabled": False, "deactivated_at": datetime.now()})
    ...
    @staticmethod
    def calculate_bid(
        strategy: BiddingStrategy, historical_data: Dict[str, Any]
    ) -> float:
        """计算最优出价"""
        ...
        if strategy.strategy_type == "ocpa":
            ...
            if historical_data.get("avg_cpa"):
                # 基于历史数据调整出价
                adjustment = (
                    target_cpa / historical_data["avg_cpa"]
                ) * strategy.bid_adjustment_factor
            ...
            if historical_data.get("avg_cpc"):
                adjustment = (
                    target_cpc / historical_data["avg_cpc"]
                ) * strategy.bid_adjustment_factor
            ...
            if historical_data.get("avg_roas"):
                adjustment = (
                    target_roas / historical_data["avg_roas"]
                ) * strategy.bid_adjustment_factor
                bid = adjustment * historical_data.get("avg_cpc", 2.0)
            else:
                bid = min_bid + (max_bid - min_bid) * 0.5
    ...
    def update_performance_metrics(
        db: Session, strategy_id: int, metrics: Dict[str, Any]
    ) -> BiddingStrategy:
        """更新策略效果指标"""
        strategy = (
            db.query(BiddingStrategy).filter(BiddingStrategy.id == strategy_id).first()
        )
        if not strategy:
        ...
        try:
            ...
            if strategy.total_conversions > 0:
                ...
                strategy.avg_roas = (
                    (metrics.get("conversion_value", 0) / strategy.total_cost)
...
class BiddingRuleService:
    ...
    def get(db: Session, id: int, tenant_id: int) -> Optional[BiddingRule]:
        """获取单个出价规则"""
        return (
            db.query(BiddingRule)
            .filter(and_(BiddingRule.id == id, BiddingRule.tenant_id == tenant_id))
            .first()
        )
    ...
    def get_multi(
        ...
        """获取出价规则列表"""
        query = db.query(BiddingRule).filter(BiddingRule.tenant_id == tenant_id)
    ...
    @staticmethod
    def update(
        db: Session, db_obj: BiddingRule, obj_in: BiddingRuleUpdate
    ) -> BiddingRule:
        """更新出价规则"""
        try:
            update_data = obj_in.dict(exclude_unset=True)
            ...
            if "conditions" in update_data:
                update_data["conditions"] = json.dumps(
