...
from sqlalchemy import Column, Integer, String, DateTime, Float, Boolean, ForeignKey, Text, JSON, BigInteger
...
class BiddingStrategy(Base):
    ...
    strategy_type = Column(String(50), nullable=False, comment='策略类型：ocpa, ocpc, roas')
...
class BiddingRule(Base):
    ...
    id = Column(Integer, primary_key=True, index=True)
    tenant_id = Column(Integer, ForeignKey('tenants.id'), nullable=False, index=True)
    strategy_id = Column(Integer, ForeignKey('bidding_strategies.id'), nullable=False, index=True)
    ...
    def __repr__(self):
        return f"<BiddingRule(id={self.id}, name={self.rule_name}, type={self.rule_type})>"
