...
class AudienceTargeting(Base):
    ...
    targeting_type = Column(
        String(50), nullable=False, comment="定向类型：interest, behavior, custom"
    )
    targeting_value = Column(Text, nullable=False, comment="定向值，JSON格式")
    is_include = Column(
        Boolean, default=True, comment="包含或排除：True=包含，False=排除"
    )
...
class DeviceTargeting(Base):
    ...
    device_type = Column(
        String(50), nullable=True, comment="设备类型：phone, tablet, all"
    )

    # 网络类型
    network_type = Column(
        String(50), nullable=True, comment="网络类型：WiFi, 4G, 5G, All"
    )
...
class GeoTargeting(Base):
    ...
    targeting_type = Column(
        String(50),
        nullable=False,
        comment="地域类型：province, city, district, business_area, lbs",
    )
    geo_level = Column(
        Integer, nullable=False, comment="地域级别：1=省，2=市，3=区，4=商圈，5=LBS"
    )
...
class TimeTargeting(Base):
    ...
    # 定向类型
    targeting_type = Column(
        String(50), nullable=False, comment="时间类型：hour, day, week, custom"
    )
...
class EnvironmentTargeting(Base):
    ...
    app_environment = Column(
        String(50), nullable=True, comment="App环境：production, test, all"
    )
