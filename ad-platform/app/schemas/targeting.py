...
class AudienceTargetingBase(BaseModel):
    ...
    targeting_type: str = Field(
        default=None, description="定向类型：interest, behavior, custom"
    )
    targeting_value: Dict[str, Any] = Field(
        default=None, description="定向值，JSON格式"
    )
...
class GeoTargetingBase(BaseModel):
    ...
    targeting_type: str = Field(
        default=None,
        description="地域类型：province, city, district, business_area, lbs",
    )
    geo_level: int = Field(
        default=None, description="地域级别：1=省，2=市，3=区，4=商圈，5=LBS"
    )
...
class TimeTargetingBase(BaseModel):
    ...
    targeting_type: str = Field(
        default=None, description="时间类型：hour, day, week, custom"
    )
