...
class BiddingStrategyBase(BaseModel):
    ...
    strategy_type: Literal["ocpa", "ocpc", "roas"] = Field(
        default=None, description="策略类型"
    )
...
class BiddingRuleBase(BaseModel):
    ...
    rule_type: Literal["hourly", "daily", "weekly", "custom"] = Field(
        default=None, description="规则类型"
    )
    adjustment_type: Literal["percentage", "fixed"] = Field(
        default=None, description="调整类型"
    )
